...
在代理式搜索(Agentic Search)的评估中，我们选择使用学术上的多跳检索数据集作为基准数据集，主要基于以下考虑：

1. **可比性需求**: 不同的记忆抽取模型产生的候选集各不相同，直接比较不同方法在这些候选集上的表现缺乏公平性和可比性。使用标准化的多跳检索数据集可以确保所有方法在相同的数据基础上进行评估。

2. **基准数据集的选择**: 我们采用了包括IRCOT、Search-R1等agentic search比较著名的pipeline所使用的MuSiQue、2WikiMultiHopQA和HotpotQA数据集。这些数据集具有以下特点：
   - **MuSiQue**: 包含多跳推理问题的数据集，需要多轮检索才能找到答案
   - **2WikiMultiHopQA**: 基于维基百科的多跳问答数据集，需要跨多个文档进行推理
   - **HotpotQA**: 热门的多跳问答数据集，包含bridge和comparison两种类型的多跳问题

3. **消融实验的可行性**: 使用这些标准数据集作为基准，我们可以方便地进行消融实验，测试我们改进内容的提升效果。

4. **与现有工作的对比**: 这些数据集已被多篇agentic search相关论文使用，便于我们与现有工作进行对比。

## 💡 系统意义

### Agentic Search改进设计的动机

本系统的改进设计源于实际应用中的性能瓶颈问题。在EverMemOS 进行smoke test时，我们发现系统在回复生成过程中，耗费在agentic search上的时间占比过高，这成为影响整体响应速度的主要瓶颈。

**面临的挑战：**

1. **时间开销问题**: Agentic search作为系统中的关键组件，其多轮迭代检索过程虽然能够提高准确性，但同时也带来了显著的时间成本，导致用户体验下降。

2. **端侧部署需求**: 作为落地应用，系统需要在算力资源有限的端侧设备上部署，这对系统的性能提出了更高的要求。

**改进目标：**

我们的目标是设计更高效的agentic search方法，在降低时耗的同时保证准确率，具体包括：

- **优化检索策略**: 减少不必要的检索轮次，提高每轮检索的精准度
- **提升检索效率**: 改进模型架构和算法，降低计算复杂度
- **平衡性能与准确率**: 在保证检索质量的前提下，尽可能减少时间开销

这一改进设计不仅能够提升系统在实际应用中的响应速度，也为在资源受限的端侧设备上部署agentic search提供了可行的解决方案。
...
```mermaid
graph TD
    Start[开始单次查询] --> LoadModel[加载训练好的检索模型]
    LoadModel --> LoadIndex[构建/加载文档索引]
    LoadIndex --> GetQuery[接收用户查询]
    GetQuery --> Hop1[第一跳检索]
